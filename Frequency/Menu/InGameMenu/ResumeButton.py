...
class ResumeButton(StartMenuItem):
    ...
    def __init__(self, offset: Vector2, oldState, image: Surface=None, hover: Surface=None, rect=None, newState=None):
        image = image if image is not None else pygame.image.load('images/buttons/resumeButton.png').convert_alpha()
        hover = hover if hover is not None else pygame.image.load('images/buttons/resumeButtonHover.png').convert_alpha()
